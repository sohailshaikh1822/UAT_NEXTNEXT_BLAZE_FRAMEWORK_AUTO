...
public class LinkDefectPage extends BasePage {
    ...
    public void verifyDefectLinkedToTRNotification(String defectId, String trId) {
        ...
        String expectedRegex =
                "'" + defectId + "'\\s+is\\s+Unlinked\\s+to\\s+'"
                        + trId + "'\\s+by\\s+"
                        + userName.replace(" ", "\\s+")
                        + "\\.";
    ...
    public void verifyDefectUnlinkedFromTRNotification(String defectId, String trId) {
        ...
        while (System.currentTimeMillis() < endTime) {
            ...
            try {
                ...
                for (WebElement element : notifications) {
                    ...
                    if (text.contains(defectId) && text.contains(trId) && text.contains("unlinked from")) {

                        if (!text.matches(expectedRegex)) {
                            throw new AssertionError(
                                    "Defect unlink notification format mismatch.\nExpected: "
                                            + expectedRegex +
                                            "\nActual: " + text
                            );
                        }
                        ...
                        System.out.println(
                                "Defect unlinked notification verified successfully: " + text
                        );
                        return;
                    }
                }
        ...
        throw new AssertionError(
                "Defect unlinked notification not found for Defect: "
                        + defectId + " and TR: " + trId
        );
    }
